...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for ((keyA, valueA) in mapA) {
        map[keyA] = valueA                             // Копирование mapA в map
        for ((keyB, valueB) in mapB)
            if (keyA == keyB && valueA != valueB) {    // Проверка на совпадение элементов а и b
                map[keyA] = "$valueA, $valueB";
            }
    }
    var count = 0;
    for ((keyB, valueB) in mapB) {                     // Добавление недостающих элементов b
        for ((key, value) in map) {
            count++;
            if (key == keyB)
                break
            if (count == map.size && key != keyB)
                map[keyB] = valueB;
        }
        count = 0;
    }
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    ...
    for (item in stockPrices) {
        ...
        if (exist) {
            //var = item.second + map[item.first];
        }
    }
    return map;
}
