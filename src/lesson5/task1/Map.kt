...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((k, v) in grades)
        map.getOrPut(v) { mutableListOf() }.add(k)
    return map
}
...
fun averageStockPrice(stockPrices: List<Pair<String, Double>>): Map<String, Double> {
    val mapSum = mutableMapOf<String, Double>()
    val mapCount = mutableMapOf<String, Int>()
    for ((first, second) in stockPrices) {
        if (mapSum.containsKey(first)) {
            mapSum[first] = mapSum[first]!! + second
            mapCount[first] = mapCount[first]!! + 1
        } else {
            mapSum[first] = second
            mapCount[first] = 1
        }
    }
    return mapSum.mapValues { it.value / mapCount[it.key]!! }
}
...
fun propagateHandshakes(friends: Map<String, Set<String>>): Map<String, Set<String>> {
    ...
    for ((k, v) in friends)
        mapFriends[k] = false to v.toMutableSet()
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    val array = Array(treasures.size + 1) { Array(capacity + 1) { 0 to setOf<String>() } }
    var i = 1
    for ((key, value) in treasures) {
        for (j in 1..capacity) {
            if (j < value.first)
                ...
                if (array[i - 1][j].first > array[i - 1][j - value.first].first + value.second)
                    array[i][j] = array[i - 1][j]
                else
                    array[i][j] = array[i - 1][j - value.first].first + value.second to array[i - 1][j - value.first].second + setOf(key)
            }
        }
        i++
    }
    return array[treasures.size][capacity].second
}
